...
TEST_CASE("infers the breach according to limits") {
  ...
  checkAndAlert(batteryChar, -3, sendToController_forTesting);
  REQUIRE(breachType == TOO_LOW);
  
  checkAndAlert(batteryChar, 30, sendToEmail_forTesting);
  ...
  checkAndAlert(batteryChar, 48, sendToEmail_forTesting);
  REQUIRE(breachType == TOO_HIGH);
